...
The pond detail page now uses a bounded protected read model for `water_quality_detail_read`, so the latest single-record detail read stays readable through a preview path but only loads the protected `GET /water-quality/:id` surface in Keycloak mode when forwarded auth/current-session are both sufficient.
...
In Keycloak mode, `water_quality_detail_read` also requires forwarded auth and a current-session that resolves to an authenticated user before the protected single-record read will load. In disabled and local modes, the non-alert read summary reports a bounded bypass path so local development stays usable.
...
If `water_quality_detail_read`, `ponds_update`, `water_quality_create`, `water_quality_update`, `feed_create`, `tasks_create`, or `ponds_create` is blocked in Keycloak mode, check:
- `Forwarded auth`
