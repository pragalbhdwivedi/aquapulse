# Tasks Local HTTP Cutover

This runbook explains the tasks-only opt-in HTTP path. AquaPulse still defaults to mock and in-memory runtime unless you explicitly enable the tasks cutover.

## Default behavior

- The web app stays mock-backed by default.
- The API keeps using in-memory adapters by default.
- Nothing in this runbook changes the global runtime for other modules.

## Enable tasks-only HTTP mode in web

Set these env vars before starting `apps/web`:

```bash
NEXT_PUBLIC_AQUAPULSE_WEB_ENABLE_FETCH_HTTP=true
NEXT_PUBLIC_AQUAPULSE_WEB_TASKS_MODE=http
NEXT_PUBLIC_AQUAPULSE_WEB_TASKS_HTTP_TRANSPORT=proxy
AQUAPULSE_WEB_LOCAL_API_BACKEND_URL=http://localhost:4000
```

If you prefer direct transport instead of the local bridge:

```bash
NEXT_PUBLIC_AQUAPULSE_WEB_TASKS_HTTP_TRANSPORT=direct
NEXT_PUBLIC_AQUAPULSE_WEB_TASKS_HTTP_BASE_URL=http://localhost:4000
```

## Backend expectations

Start `apps/api` with the existing tasks Postgres adapter configuration enabled if you want a true HTTP plus Postgres cutover. The `/runtime` page and `/api/diagnostics/runtime` will show whether tasks is really using `in-memory` or `postgres`.

## Local bridge path

When transport is `proxy`, the web app sends tasks requests through:

- `/api/tasks`

The Next.js bridge forwards those requests to the backend target from `AQUAPULSE_WEB_LOCAL_API_BACKEND_URL`.

## Verify the cutover

Run the web app and API, then execute:

```bash
corepack pnpm tasks:verify-runtime
```

Useful verifier env vars:

```bash
AQUAPULSE_TASKS_VERIFY_WEB_BASE_URL=http://localhost:3000
AQUAPULSE_TASKS_VERIFY_API_BASE_URL=http://localhost:4000
AQUAPULSE_TASKS_VERIFY_EXPECT_BACKEND_ADAPTER=postgres
AQUAPULSE_TASKS_VERIFY_TASK_ID=task-1
AQUAPULSE_TASKS_VERIFY_POND_ID=pond-1
```

The verifier checks:

- backend runtime diagnostics
- tasks list through the web bridge
- task detail through the web bridge
- task create through the web bridge
- task update through the web bridge

## What remains default

- global web runtime: mock
- API persistence default: in-memory
- alerts, water-quality, and feed modes: unchanged unless separately enabled
