...
export function readSmokeDatabaseConfig(env = process.env) {
  const sslMode =
    env.AQUAPULSE_PONDS_SMOKE_DB_SSL_MODE ??
  ...
  return {
    host:
      env.AQUAPULSE_PONDS_SMOKE_DB_HOST ??
      ...
      "localhost",
    port: parseNumber(
      env.AQUAPULSE_PONDS_SMOKE_DB_PORT ??
    ...
    database:
      env.AQUAPULSE_PONDS_SMOKE_DB_NAME ??
      ...
      "aquapulse",
    user:
      env.AQUAPULSE_PONDS_SMOKE_DB_USER ??
      ...
      "aquapulse",
    password:
      env.AQUAPULSE_PONDS_SMOKE_DB_PASSWORD ??
