...
describe("Client runtime config and invocation registry", () => {
  ...
  it("surfaces bounded auth-forwarding diagnostics when keycloak mode has a forwardable token", () => {
    ...
    expect(diagnostics.tertiaryNonAlertsProtectedSliceLabel).toBe("ponds_update");
    ...
    expect(diagnostics.octonaryNonAlertsProtectedSliceLabel).toBe("ponds_create");
    expect(diagnostics.octonaryNonAlertsProtectedSliceEnforced).toBe(true);
